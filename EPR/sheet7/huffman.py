...
class HuffmanTree:
    ...
    @staticmethod
    def decode(text: str, code: dict) -> str:
        """
    ...
    @staticmethod
    def encode(text: str, code: dict) -> str:
        result = ""
        for letter in text:
    ...
    @staticmethod
    def compute_code(tree: InternalNode, largest_word=""):
        code = dict()
    ...
    def create_leafs(text: str) -> [Leaf]:
        leafs = map(lambda t: Leaf(t[0], t[1]), Counter(text).items())
        return map(lambda l: Leaf(l.value, l.weight / len(text)), leafs)
